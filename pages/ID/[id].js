...
import { collection, getDocs,getDoc, query, where,onSnapshot,orderBy,serverTimestamp,updateDoc,arrayUnion, arrayRemove,addDoc } from "firebase/firestore"
...
// })
  ...
  return (
    ...
    <Grid container spacing={3}>
      ...
      :
      <div>
      <img src="/noimage.jpg" alt="" width="200" height="250" />
      </div>
      }
      ...
      <div>
      <br />
      <Typography variant="h5" color="primary" onClick={()=>{window.open(custom1get, '_blank')}}>#{hash1get}</Typography>
      </div>
      }
      ...
      <Typography variant="h5" color="primary" onClick={()=>{window.open(custom2get, '_blank')}}>#{hash2get}</Typography>
      ...
      <Grid item xs={12} md={8} lg={9}>
       ...
       {ninja[0].username}
        ...
        <span className={classes.imagemargin} onClick={handleOnArraytest}>followers:{followers?followers.length:"0"}</span> 
        &nbsp;
    ...
    <Grid item xs={12} md={12} lg={12} className={classes.balancer}>
        ...
        {ninja.map((item,index) => (
          ...
          <img src={`http://img.youtube.com/vi/${item.youtubeId}/maxresdefault.jpg`} alt="" width='100%' onClick={()=>{window.open("https://www.youtube.com/watch?v="+item.youtubeId, '_blank')}}　/>
